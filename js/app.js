        ...
        if (document.querySelector("[data-fp]")) {
            ...
            const checker = function() {
                if (breakpoint.matches) {
                    ...
                    return flsModules.fullpage = new FullPage(document.querySelector("[data-fp]"), "");
                } else {
...
***************************************************************************** */
        ...
        da.init();
        function dropDownActiv(btn, submenu, activeClass) {
            const menuItems = document.querySelectorAll(btn);
            menuItems.forEach((item => {
                item.addEventListener("click", (function() {
                    this.closest(submenu).classList.toggle(activeClass);
                    menuItems.forEach((otherItem => {
                        if (otherItem !== this) otherItem.closest(submenu).classList.remove(activeClass);
                    }));
                }));
            }));
            document.addEventListener("click", (e => {
                const targetElement = e.target;
                if (!targetElement.closest(btn) && document.querySelectorAll(submenu + "." + activeClass).length > 0) removeClasses(document.querySelectorAll(submenu + "." + activeClass), activeClass);
            }));
        }
        dropDownActiv(".bigmenu__button", ".bigmenu", "bigmenu-open");
        ...
        function buildLocalNavigation() {
            ...
            window.addEventListener("scroll", (function() {
                ...
                sections.forEach((current => {
                    ...
                    const link = document.querySelector(`[data-goto="#${sectionId}"]`);
                    if (link != null) if (scroll_y > section_top && scroll_y <= section_top + section_height) link.classList.add("active"); else link.classList.remove("active");
                }));
            }));
        }
